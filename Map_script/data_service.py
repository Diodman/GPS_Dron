...
class DataService:
    def __init__(self, cache_dir="cache_data"):
        ...
        # Overpass endpoints: fallback when one instance is down/slow
        # (OSM/Overpass часто нестабильны; перебор инстансов даёт заметно больше успешных загрузок).
        self._overpass_urls = [
            ...
            # HTTP fallback (иногда HTTPS режется/ломается провайдером/прокси)
    ...
    def _try_load_road_graph_with_overpass_fallback(
        self,
        *,
        boundary_for_roads,
        city_name: str,
        ...
        """
        Пытается загрузить road_graph, перебирая несколько Overpass-инстансов.
        Возвращает первый успешный граф или None.
        """
        # Lazy import to keep module load cheap
        import osmnx as ox

        # OSMnx settings vary by version; snapshot all known knobs
        original_overpass_url = getattr(ox.settings, "overpass_url", None)
        original_overpass_endpoint = getattr(ox.settings, "overpass_endpoint", None)
        original_overpass_base_url = getattr(ox.settings, "overpass_base_url", None)
        original_timeout = getattr(ox.settings, "requests_timeout", None)
        original_http_timeout = getattr(ox.settings, "http_timeout", None)

        # чуть меньше шанс зависнуть на одном инстансе
        try:
            ox.settings.requests_timeout = 90
        except Exception:
            pass
        try:
            ox.settings.http_timeout = 90
        except Exception:
            pass

        def _attempt_once():
            road_graph_local = None
            # 1) polygon (лучший охват)
            if boundary_for_roads is not None:
                try:
                    ...
                    road_graph_local = ox.graph_from_polygon(boundary_buffered, custom_filter=road_filter, simplify=simplify)
                    if road_graph_local is not None and len(road_graph_local.nodes) > 0:
                        return road_graph_local
            ...
            for q in query_variants:
                ...
                for which in [1, 2, 3]:
                    try:
                        road_graph_local = ox.graph_from_place(q, custom_filter=road_filter, simplify=simplify, which_result=which)
                        if road_graph_local is not None and len(road_graph_local.nodes) > 0:
                            return road_graph_local
                    except Exception as e:
        ...
        try:
            for url in list(self._overpass_urls or []):
                try:
                    # Depending on OSMnx version different attrs are used.
                    if hasattr(ox.settings, "overpass_url"):
                        ox.settings.overpass_url = url
                    if hasattr(ox.settings, "overpass_endpoint"):
                        ox.settings.overpass_endpoint = url
                    if hasattr(ox.settings, "overpass_base_url"):
                        ox.settings.overpass_base_url = url.replace("/api/interpreter", "")
                except Exception:
                    pass
                try:
                    cur = getattr(ox.settings, "overpass_url", None) or getattr(ox.settings, "overpass_endpoint", None) or getattr(ox.settings, "overpass_base_url", None)
                except Exception:
                    cur = None
                self.logger.info("Overpass: пробуем %s (osmnx=%s)", url, cur)
                g = _attempt_once()
                if g is not None and len(g.nodes) > 0:
                    self.logger.info("Overpass: успех на %s (узлов: %s)", url, len(g.nodes))
                    return g
        finally:
            # restore settings
            try:
                if original_overpass_url is not None:
                    ox.settings.overpass_url = original_overpass_url
            except Exception:
                pass
            try:
                if original_overpass_endpoint is not None and hasattr(ox.settings, "overpass_endpoint"):
                    ox.settings.overpass_endpoint = original_overpass_endpoint
            except Exception:
                pass
            try:
                if original_overpass_base_url is not None and hasattr(ox.settings, "overpass_base_url"):
                    ox.settings.overpass_base_url = original_overpass_base_url
            except Exception:
                pass
            try:
                ...
                pass
            try:
                if original_http_timeout is not None and hasattr(ox.settings, "http_timeout"):
                    ox.settings.http_timeout = original_http_timeout
            except Exception:
                pass
        return None
    ...
    def _download_city_data(self, city_name, redis_key: str | None = None, clusters_redis_key: str | None = None, *, network_type: str = 'drive', simplify: bool = True, load_no_fly_zones: bool = True):
        ...
        try:
            ...
            # Основной способ: перебор Overpass-инстансов для устойчивости.
            road_graph = self._try_load_road_graph_with_overpass_fallback(
                boundary_for_roads=boundary_for_roads,
                city_name=city_name,
