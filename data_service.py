    ...
    def get_station_candidates(
        ...
        if station_type in ("garage", "to"):
            try:
                ...
                landuse_union = None
            # Fallback: если OSM не отдаёт landuse-зоны, используем промышленные/складские/гаражные здания
            # (это соответствует существующей логике station_placement: всё равно нужны кандидаты в «пром» контуре).
            if landuse_union is None or getattr(landuse_union, "is_empty", True):
                self.logger.warning(
                    "Гараж и ТО: нет полигонов landuse (industrial / garages / brownfield) — включаем fallback по зданиям"
                )
                landuse_union = None
        ...
        for idx, row in b.iterrows():
            ...
            if station_type == "rooftop":
                ...
                if tag not in self._GARAGE_TO_ALLOWED_BUILDING_TAGS:
                    continue
                pt = Point(lon, lat)
                # Если landuse_union есть — строго внутри зон; если landuse_union нет — fallback допускает здания без зон.
                if landuse_union is not None:
                    try:
                        if not (
                ...
                rows.append(
                    {
                        "geometry": Point(lon, lat),
                        "source": "industrial" if landuse_union is not None else "fallback_no_landuse",
                        "osm_index": idx,
                        "area_m2": float(area),
                    }
                )
        ...
        # Fallback #2: если landuse зон нет и по тегам зданий кандидатов тоже нет —
        # берём крупнейшие допустимые здания вне no-fly (лучше, чем пустой список).
        if station_type in ("garage", "to") and not rows and landuse_union is None:
            try:
                bb = b.copy()
                # исключаем очевидно непригодные типы
                if "building" in bb.columns:
                    bb["__btag"] = bb["building"].astype(str).str.lower().str.strip()
                    bb = bb[~bb["__btag"].isin(self._GARAGE_TO_EXCLUDED_BUILDING_TAGS)].copy()
                # сортируем по площади и берём топ
                bb = bb.sort_values(by="area_m2", ascending=False)
                top = bb.head(40)
                for idx, row in top.iterrows():
                    ...
                    rows.append(
                        {
                            "geometry": Point(float(c.x), float(c.y)),
                            "source": "fallback_any_building",
                            "osm_index": idx,
                            "area_m2": float(row.get("area_m2", 0) or 0),
                        }
                    )
                if rows:
                    self.logger.warning(
                        "Кандидаты %s: fallback по крупнейшим зданиям вне no-fly (без landuse): %s",
                        station_type,
                        len(rows),
                    )
            except Exception as ex:
                self.logger.warning("Кандидаты %s (fallback_any_building): %s", station_type, ex)
